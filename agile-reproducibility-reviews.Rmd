...
The code of this document is published on GitHub in the repository [reproducible-agile/reviews-2026](https://github.com/reproducible-agile/reviews-2026), where you can inspect the R code in the file `agile-reproducibility-reviews.Rmd` and find instructions for using this document to support the reproducibility review.
...
If you read this report publicly online, you are reading the version without private information.
If you are interested in the full version, please contact the AGILE chairs or the authors of this report.
...
# EasyChair conference IDs — there are two different IDs for the same conference:
#   36110000 — the "editor" (PC member) ID, shown in URLs when logged in as reviewer/PC member
#   36190656 — the "conference chair" ID, used for full chair-level access (submissions, reviews, author contacts)
# Navigate via https://easychair.org/my2/conference?conf=agile2026 and extract the ID from the submissions link
conference_id <- "36190656"
```

```{r easychair_login, echo=params$private_info, results=if(!params$private_info) "hide" else "markup", warning=params$private_info}
...
if(is.na(Sys.getenv("easychair_username", unset = NA)) || is.na(Sys.getenv("easychair_password", unset = NA))) {
  warning("EasyChair credentials not found. Provide login details (e.g., in a file `.Renviron`) in environment variables",
...
# Reusable login function — can be called again if session expires mid-render
ec_login <- function() {
  # Clear any stale session cookies (important when re-rendering from an active RStudio session)
  httr::handle_reset("https://easychair.org")
  ...
  cat("Login response status:", httr::status_code(resp), "\n")
  login_json <- tryCatch(
  ...
  if(!is.null(redirect_path)) {
    cat("Following redirect to:", redirect_path, "\n")
    httr::GET(url = paste0("https://easychair.org", redirect_path), ec_ua)
  }
...
ec_ensure_session <- function() {
  check <- httr::GET(url = paste0("https://easychair.org/conferences2/submissions?a=", conference_id), ec_ua)
  if(grepl("signin", check$url)) {
    cat("EasyChair session expired, re-logging in...\n")
    login_response <<- ec_login()
  }
}
...
if(!is.na(Sys.getenv("easychair_username", unset = NA)) && !is.na(Sys.getenv("easychair_password", unset = NA))) {
  tryCatch({
    ...
    warning("EasyChair login failed: ", conditionMessage(e),
            ". Falling back to cached data if available.")
    easychair_logged_in <<- FALSE
  })
} else {
  cat("Skipping EasyChair login (no credentials available)\n")
}
```
...
submission_cache_file <- file.path(submissions_path, "submission_table.csv")

if(!easychair_logged_in) {
  if(file.exists(submission_cache_file)) {
    cat("EasyChair login not available. Loading submission table from cache:", submission_cache_file, "\n")
    submission_table <- readr::read_csv(submission_cache_file, show_col_types = FALSE)
    submission_table <- submission_table %>%
      mutate(id = str_pad(`#`, width = 3, side = "left", pad = "0")) %>%
      mutate_if(is.character, list(~na_if(.,"")))
  } else {
    cat("EasyChair login not available and no cached data found. Creating empty submission table.\n")
...
# Use the AJAX endpoint to enable Decisions and Keywords columns
fields_response <- httr::POST(url = paste0("https://easychair.org/conferences2/submission_field_hide_x?a=", conference_id),
...
# check we got the actual page and were not redirected to login; retry once after re-login
if(grepl("signin", submission_page$url)) {
  cat("Session expired, re-logging in...\n")
  login_response <- ec_login()
  fields_response <- httr::POST(url = paste0("https://easychair.org/conferences2/submission_field_hide_x?a=", conference_id),
                           body = list(a = conference_id, keywords = "on", decision = "on", time = "on"), ec_ua)
  submission_page <- httr::GET(url = submissions_url, ec_ua)
  if(grepl("signin", submission_page$url)) {
...
# cache submission table for offline/public rendering
readr::write_csv(submission_table, file = submission_cache_file)
cat("Saved submission table cache to", submission_cache_file, "\n")
...
if(easychair_logged_in && nrow(submission_table) > 0) {
  for (i in 1:nrow(submission_table)) {
...
} else if(!easychair_logged_in) {
  cat("Skipping EasyChair PDF downloads (not logged in). Using locally available PDFs.\n")
}
...
The report should be based on one of the templates in the [`report-template` folder](https://github.com/reproducible-agile/reviews-2025/tree/main/report-template) in the repository for the 2026 reproducibility reviews.
...
```{r accepted_fp_files_download_easychair, echo=FALSE}
review_files <- review_data %>%
  filter(type == "Full paper") %>%
  dplyr::filter(Decisions == "ACCEPT") %>%
  select(id, submission_id, Decisions, Authors, Title, file, paper)

if(easychair_logged_in) {
  # re-download all accepted full paper PDFs to make sure we have latest copies
  ...
  cat("Skipping EasyChair download of accepted paper PDFs (not logged in). Using locally available files.\n")
}
```
...
# Re-login if session expired during the long PDF processing phases above
ec_ensure_session()
...
# Re-login if session expired since the review retrieval loop
ec_ensure_session()
...
if (params$private_info && interactive()) {
  # put accepted full papers to private share (requires interactive Google auth)
  library("googledrive")
  googledrive::drive_auth()
  submissions_and_reviews <- "https://drive.google.com/drive/folders/12YeZfdhWB8t6cckSSIkVwTj7Hn39kUjO"
...
1. Distribute the reproductions among the committee members
1. Support committee members during reproductions, give feedback, advise on problems, ...
...
1. Contact conference chairs to coordinate on the presentation of the reproducibility review overview at the conference, e.g., at the AGILE meeting or the closing session
