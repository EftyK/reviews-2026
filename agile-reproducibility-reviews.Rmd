...
if(is.null(login_response)) {
  login_response <- httr::POST(url = "https://easychair.org/account2/signin_x",
                         body = list(name = Sys.getenv("easychair_username"),
                                     password = Sys.getenv("easychair_password")),
                         encode = "form")
  #cat("Logged in at EasyChair: ", toString(login_response))
...
# get this from the URL when logged into EasyChair, e.g., https://easychair.org/conferences2/submissions?a=34212104
conference_id <- "34212104"
# regular conference ID is "33640409", the above seems to be only for track chairs
```
...
submission_page <- httr::GET(url = paste0("https://easychair.org/conferences2/submissions?a=", conference_id))
...
fields_response <- httr::POST(url = "https://easychair.org/conferences/submission_field_hide_x.cgi?a=34091123",
                         body = list(a = "34091123",
                                     keywords = "on",
                                     decision = "on",
                                     time = "on",
                                     visible = "65094",
...
xml_remove(xml2::xml_child(rvest::html_element(submission_table_full, "thead")))
...
#review_table <- rvest::html_table(rvest::html_elements(review_status_page, ".paperTable")[[1]], header = TRUE)
...
#review_table$internal_id <- sapply(X = rvest::html_elements(review_status_page, css = ".paperTable tr[id]"), FUN = function(row) {
...
```{r report_links, echo=FALSE, eval=params$private_info}
...
# Example page: https://easychair.org/conferences2/submission_view?a=26091618;submission=5333543
retrieve_authors <- function(id, submission_id, authors, title) {
  ...
  xml2::xml_add_child(authors_body,
                      xml2::xml_find_all(content,
                                           xpath = "//table[@class='ct_table']")[[2]]) # Authors are the second table on the page
